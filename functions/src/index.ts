...
// ── Agora voice/video call tokens ───────────────────────────────────────────
// The client used to join Agora channels with an empty token string, which
// only worked while this Agora project had no Primary Certificate set. Once
// a certificate is configured, Agora requires a real token to join — and
// joining without one stopped failing gracefully, it crashes the native SDK
// outright on iOS. Token generation has to happen server-side: the App
// Certificate must never be shipped in the client.
//
// Set the certificate per project with:
//   firebase functions:secrets:set AGORA_APP_CERTIFICATE --project dev
//   firebase functions:secrets:set AGORA_APP_CERTIFICATE --project prod
const agoraAppCertificate = defineSecret("AGORA_APP_CERTIFICATE");

// Same App ID already embedded client-side (voice_call_screen.dart /
// video_call_screen.dart) — the App ID itself isn't sensitive, only the
// certificate is.
const AGORA_APP_ID = "75426b0c60784c2ebd9ab32cfcc5288f";

const AGORA_TOKEN_EXPIRE_SECONDS = 3600;
...
export const generateAgoraToken = onCall(
  ...
  (request) => {
    ...
    const channelName = request.data?.channelName as string | undefined;
    if (!channelName || typeof channelName !== "string") {
      throw new HttpsError("invalid-argument", "Missing 'channelName'.");
    }

    // uid 0 means "valid for any uid" in Agora's token spec — every call
    // screen joins with uid: 0 (see voice_call_screen.dart/
    // video_call_screen.dart), so one token generated by the caller works
